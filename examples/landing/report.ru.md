...
По умолчанию получается один автономный HTML-файл без сервера, удалённой среды выполнения и авторского JavaScript.
:::

::::cards
:::card{title="Пишите естественно"}
Используйте Markdown, метаданные в начале файла, ограниченные включения и локальные ресурсы.
:::
...
Собирайте единым рабочим процессом, который проверяет исходник перед записью.
:::
::::

:::glossary{key="portable-boundary" term="Переносимая граница" placement="appendix"}
Источник остаётся декларативным, а пакет владеет рендерингом, взаимодействием, ограничением путей и автономным результатом.
:::
:::::
...
Испытайте реальную страницу через `file://` на компьютере и телефоне.
:::
::::
:::::
...
обработчики или внешний редактор.
:::

:::popover{title="Сведения о переносимости" trigger="Почему важен file://?"}
Открытие прямо с диска доказывает, что после сборки артефакт не зависит от сервера разработки или удалённой
среды выполнения.
:::
::::

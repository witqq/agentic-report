...
test('staged landing reaches live examples, human docs, and direct agent instructions through file URLs', async ({
  ...
  await expect(
    page.getByText('git clone --branch v0.5.0 --depth 1', { exact: false }),
  ).toBeVisible();
  ...
  await expect(
    page.getByText('npx --yes agentic-report@0.5.0 init ./my-page', { exact: false }).first(),
  ).toBeVisible();
  ...
  await expect(page.locator('body')).toContainText('npx --yes agentic-report@0.5.0 validate');

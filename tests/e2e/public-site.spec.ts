...
test('staged landing reaches live examples, human docs, and direct agent instructions through file URLs', async ({
  ...
  await expect(
    page.getByText('npx --yes agentic-report@0.2.2 init ./my-page', { exact: false }).first(),
  ).toBeVisible();
  ...
  await expect(page.locator('body')).toContainText('npx --yes agentic-report@0.2.2 validate');

...
describe('release readiness', () => {
  it('documents the public asset publish source and manual metadata inspection', async () => {
    ...
    expect(publish).toContain('npm view agentic-report@0.2.4 --json');
  ...
  it('keeps the two registry npx journeys isolated', async () => {
    ...
    for (const block of [pinned, latest]) {
      expect(block).toContain('view agentic-report@0.2.4 --json > ./npm-version.json');
    ...
    expect(
      pinnedCommands
        ...
        .every((line) => line.includes('agentic-report@0.2.4')),
    ).toBe(true);
    expect(
      latestCommands
        .filter((line) => line.includes('"$release_npx"'))

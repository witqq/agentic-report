...
3. Соберите один HTML или каталог с адресуемыми ресурсами: `build` проверяет исходник перед записью.
4. Откройте результат через `file://`; используйте `validate` или `inspect` только для точечной диагностики.
   :::

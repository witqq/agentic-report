...
When a report contains review targets, its topbar includes `Review`. Persistent review controls remain hidden
in normal reading. Selecting eligible rendered text exposes one localized **Create note** action beside the
native highlight; activation snapshots the range and opens the existing target editor with its exact quote.
The range may stay inside one review target or end in a later target. Both endpoints must belong to the same
report article. Empty, whitespace-only, oversized, outside-report, reversed, package-control, or
unreconstructable ranges expose no action or fail import without replacing current state. Releasing `Shift`
after a keyboard selection focuses the action; pointer and touch selection use the same captured state.

The `Review` control enables whole-block target buttons and opens the workspace directly on desktop; mobile
keeps the report available for target selection before opening its modal sheet. Tight-list paragraphs do not
become separate targets because they render without their own paragraph element—the containing list remains
reviewable.
...
resolved/reopened state. A compact target indicator opens the whole-block thread and a separate control
resolves it; the panel lists every current block thread and selected-text note. Canonical version-3
`review.json` download exports all of them together. Exact-revision import validates every current target,
offset, range order, and quote against the rendered DOM before swapping state. It rejects oversized,
malformed, version-1, stale, foreign, non-current, or mismatched selected-text data without replacing active
state; valid version-2 whole-block import remains supported.
...
| Review Workspace    | Normal reading has no persistent target controls. A valid text range exposes one localized **Create note** action; the panel shows the exact quote, messages, resolution state, every current note, local import, and one all-notes export.                                                                                                                                                                                             | Native selection plus `Shift` release focuses **Create note**; panel buttons, textarea, import, resolve/reopen, and export use normal keyboard behavior. Desktop is non-modal and mobile uses native modal containment/return. | Pointer/touch selection exposes the same action near the highlight. Empty, whitespace-only, oversized, outside-report, control, reversed, or mismatched ranges create nothing. |

...
```sh
git tag -a v0.4.4 -m "agentic-report 0.4.4"
git push origin v0.4.4
gh release create v0.4.4 "$candidate_path" --title "agentic-report 0.4.4" --notes-file ./release-notes.md
```
...
Continue only when the workflow succeeds and npm reports `latest` and `version` as `0.4.4`. Do not run
...
Confirm `release.json` reports package `0.4.4` and the accepted merge commit, the landing returns HTML over

...
```sh
git tag -a v0.10.0 -m "agentic-report 0.10.0"
git push origin v0.10.0
gh release create v0.10.0 "$candidate_path" --title "agentic-report 0.10.0" --notes-file ./release-notes.md
```
...
Continue only when the workflow succeeds and npm reports `latest` and `version` as `0.10.0`. Do not run
...
Confirm `release.json` reports package `0.10.0` and the accepted merge commit, the landing returns HTML over

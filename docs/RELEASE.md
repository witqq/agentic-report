...
candidate is public. Release `0.2.1` is not proven by an installed tarball alone: GitHub, npm, registry-clean
...
- package, installed CLI `--version`, skill and plugin metadata: `0.2.1`;
...
the npm registry serves `0.2.1` and must be labelled accordingly.
...
git status --short
git rev-parse HEAD
git tag -a v0.2.1 -m "agentic-report 0.2.1"
...
`[Made with Moira](https://github.com/witqq/mcp-moira)`.

## Publish the inspected tarball

Upload the exact locally accepted `agentic-report-0.2.1.tgz` bytes as the asset of the verified GitHub
Release `v0.2.1`. Download that public asset into a new empty directory, verify its SHA-256 equals the local
...
asset_root="$(mktemp -d)"
curl --fail --location --proto '=https' --tlsv1.2 --output "$asset_root/agentic-report-0.2.1.tgz" "$release_asset_url"
test "$(shasum -a 256 "$asset_root/agentic-report-0.2.1.tgz" | awk '{print $1}')" = "$candidate_sha256"
...
env -i PATH="$PATH" HOME="$metadata_root/home" npm_config_cache="$metadata_root/npm-cache" npm_config_userconfig="$metadata_root/user.npmrc" npm_config_globalconfig="$metadata_root/global.npmrc" npm view agentic-report@0.2.1 --json > "$asset_root/npm-version.json"
...
```sh
cd "$pinned_work"
env -i PATH="$release_path" HOME="$pinned_home" npm_config_cache="$pinned_cache" npm_config_userconfig="$pinned_userconfig" npm_config_globalconfig="$pinned_globalconfig" "$release_npm" view agentic-report@0.2.1 --json > ./npm-version.json
cat ./npm-version.json
env -i PATH="$release_path" HOME="$pinned_home" npm_config_cache="$pinned_cache" npm_config_userconfig="$pinned_userconfig" npm_config_globalconfig="$pinned_globalconfig" "$release_npx" --yes agentic-report@0.2.1 --version
env -i PATH="$release_path" HOME="$pinned_home" npm_config_cache="$pinned_cache" npm_config_userconfig="$pinned_userconfig" npm_config_globalconfig="$pinned_globalconfig" "$release_npx" --yes agentic-report@0.2.1 init ./pinned-page --starter landing --json
env -i PATH="$release_path" HOME="$pinned_home" npm_config_cache="$pinned_cache" npm_config_userconfig="$pinned_userconfig" npm_config_globalconfig="$pinned_globalconfig" "$release_npx" --yes agentic-report@0.2.1 validate ./pinned-page --json
env -i PATH="$release_path" HOME="$pinned_home" npm_config_cache="$pinned_cache" npm_config_userconfig="$pinned_userconfig" npm_config_globalconfig="$pinned_globalconfig" "$release_npx" --yes agentic-report@0.2.1 inspect ./pinned-page --json
env -i PATH="$release_path" HOME="$pinned_home" npm_config_cache="$pinned_cache" npm_config_userconfig="$pinned_userconfig" npm_config_globalconfig="$pinned_globalconfig" "$release_npx" --yes agentic-report@0.2.1 build ./pinned-page --output ./pinned-page.html --json
...
The unversioned journey first observes `latest=0.2.1` through its own still-empty cache, then uses only that
cache and workspace:
...
env -i PATH="$release_path" HOME="$latest_home" npm_config_cache="$latest_cache" npm_config_userconfig="$latest_userconfig" npm_config_globalconfig="$latest_globalconfig" "$release_npm" view agentic-report@0.2.1 --json > ./npm-version.json
cat ./npm-version.json

...
```sh
git tag -a v0.4.3 -m "agentic-report 0.4.3"
git push origin v0.4.3
gh release create v0.4.3 "$candidate_path" --title "agentic-report 0.4.3" --notes-file ./release-notes.md
```
...
Continue only when the workflow succeeds and npm reports `latest` and `version` as `0.4.3`. Do not run
...
Confirm `release.json` reports package `0.4.3` and the accepted merge commit, the landing returns HTML over

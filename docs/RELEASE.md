...
candidate is public. Release `0.4.1` is not proven by an installed tarball alone: GitHub, npm, registry-clean
...
- package, installed CLI `--version`, skill and plugin metadata: `0.4.1`;
...
the npm registry serves `0.4.1` and must be labelled accordingly.
...
git status --short
git rev-parse HEAD
git tag -a v0.4.1 -m "agentic-report 0.4.1"
...
Upload the exact locally accepted `agentic-report-0.4.1.tgz` bytes as the asset of the verified GitHub
Release `v0.4.1`. Download that public asset into a new empty directory, verify its SHA-256 equals the local
candidate, then dispatch the repository's trusted OIDC workflow with the tag and accepted digest. The
workflow verifies and publishes the exact canonical HTTPS asset URL. Local directories, relative/absolute paths,
...
candidate_integrity="$(node -e 'const r=require(process.argv[1]); process.stdout.write(r.tarball.integrity)' "$candidate_record")"
candidate_shasum="$(node -e 'const r=require(process.argv[1]); process.stdout.write(r.tarball.shasum)' "$candidate_record")"
release_asset_url="https://github.com/witqq/agentic-report/releases/download/v0.4.1/agentic-report-0.4.1.tgz"
asset_root="$(mktemp -d)"
curl --fail --location --proto '=https' --tlsv1.2 --output "$asset_root/agentic-report-0.4.1.tgz" "$release_asset_url"
test "$(shasum -a 256 "$asset_root/agentic-report-0.4.1.tgz" | awk '{print $1}')" = "$candidate_sha256"
gh workflow run publish-npm.yml --ref main -f tag=v0.4.1 -f sha256="$candidate_sha256"
...
env -i PATH="$PATH" HOME="$metadata_root/home" npm_config_cache="$metadata_root/npm-cache" npm_config_userconfig="$metadata_root/user.npmrc" npm_config_globalconfig="$metadata_root/global.npmrc" npm view agentic-report@0.4.1 --json > "$asset_root/npm-version.json"
...
```sh
cd "$pinned_work"
env -i PATH="$release_path" HOME="$pinned_home" npm_config_cache="$pinned_cache" npm_config_userconfig="$pinned_userconfig" npm_config_globalconfig="$pinned_globalconfig" "$release_npm" view agentic-report@0.4.1 --json > ./npm-version.json
cat ./npm-version.json
env -i PATH="$release_path" HOME="$pinned_home" npm_config_cache="$pinned_cache" npm_config_userconfig="$pinned_userconfig" npm_config_globalconfig="$pinned_globalconfig" "$release_npx" --yes agentic-report@0.4.1 --version
env -i PATH="$release_path" HOME="$pinned_home" npm_config_cache="$pinned_cache" npm_config_userconfig="$pinned_userconfig" npm_config_globalconfig="$pinned_globalconfig" "$release_npx" --yes agentic-report@0.4.1 init ./pinned-page --starter landing --json
env -i PATH="$release_path" HOME="$pinned_home" npm_config_cache="$pinned_cache" npm_config_userconfig="$pinned_userconfig" npm_config_globalconfig="$pinned_globalconfig" "$release_npx" --yes agentic-report@0.4.1 validate ./pinned-page --json
env -i PATH="$release_path" HOME="$pinned_home" npm_config_cache="$pinned_cache" npm_config_userconfig="$pinned_userconfig" npm_config_globalconfig="$pinned_globalconfig" "$release_npx" --yes agentic-report@0.4.1 inspect ./pinned-page --json
env -i PATH="$release_path" HOME="$pinned_home" npm_config_cache="$pinned_cache" npm_config_userconfig="$pinned_userconfig" npm_config_globalconfig="$pinned_globalconfig" "$release_npx" --yes agentic-report@0.4.1 build ./pinned-page --output ./pinned-page.html --json
...
The unversioned journey first observes `latest=0.4.1` through its own still-empty cache, then uses only that
cache and workspace:
...
env -i PATH="$release_path" HOME="$latest_home" npm_config_cache="$latest_cache" npm_config_userconfig="$latest_userconfig" npm_config_globalconfig="$latest_globalconfig" "$release_npm" view agentic-report@0.4.1 --json > ./npm-version.json
cat ./npm-version.json
...
and workspace roots must remain distinct, and neither may contain a checkout link or local tarball.

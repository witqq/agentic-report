...
```sh
git tag -a v0.11.0 -m "agentic-report 0.11.0"
git push origin v0.11.0
gh release create v0.11.0 "$candidate_path" --title "agentic-report 0.11.0" --notes-file ./release-notes.md
```
...
```sh
gh workflow run publish-npm.yml --ref main -f tag=v0.11.0 -f sha256="$candidate_sha256"
...
Continue only when the workflow succeeds and npm reports `latest` and `version` as `0.11.0`. Do not run
...
Confirm `release.json` reports package `0.11.0` and the accepted merge commit, the landing returns HTML over
